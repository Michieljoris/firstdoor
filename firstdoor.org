...
** 24/2/14  .5h replacing slideshow with static image
   ...
   http://www.jqueryscript.net/social-media/jQuery-Plure gin-For-Custom-Facebook-Like-Button-Fancylike.html
...
* doing 
- Create dialog asking for:
publishedat, createdat, tags, title, comments, published
- Sync all but blog to live on linode
- Hide meta and unpublished widget in firstdoor on logged out status
  
- delete post  

-bb-server should be set not to serve certain dir unless session is verified,
like blog/unpublished
-clear post and unpublished dirs!!
